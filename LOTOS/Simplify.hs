...
simplify = everywhere $ mkT f
    ...
    f (Sequence names b (Exit vs)) = replaceExit b
        ...
        replaceExit b = gmapT (mkT replaceExit) b
    ...
    f (Interleaving (Exit vs) b) | Just merged <- everywhereM (mkM $ unifyExits vs) b = merged
    f (Interleaving a (Exit vs)) | Just merged <- everywhereM (mkM $ unifyExits vs) a = merged
...
impossibleGates gates b = gmapT (mkT $ impossibleGates gates) b
...
insertBeforeExit f results b = gmapT (mkT $ insertBeforeExit f results) b
